...
fit_statsSteps = auth2_client.intraday_time_series('activities/steps', base_date=yesterday2, detail_level='1sec')
#fit_statsHR = auth2_client.intraday_time_series('activities/heart', base_date=yesterday2, detail_level='1sec')
...
while running == True:
  ...
  time_list = []
  val_list = []
  print("Heart Intraday")
  for i in fit_statsHR['activities-heart-intraday']['dataset']:
    val_list.append(i['value'])
    time_list.append(i['time'])
    heartdf = pd.DataFrame({'Heart Rate':val_list,'Time':time_list})

    heartdf.to_csv('./data/heart'+ \
          	yesterday+'.csv', \
  ...
  for i in fit_statsHR['activities-steps-intraday']['dataset']:
    val_list.append(i['value'])
    time_list.append(i['time'])
    stepsdf = pd.DataFrame({'Step Count':val_list,'Time':time_list})

    heartdf.to_csv('./data/Steps'+ \
          	yesterday+'.csv', \
